...
async function startCamera(){
  try{
    if(!navigator.mediaDevices?.getUserMedia){
      throw new Error("このブラウザではページ内カメラを使えません");
    }
    ...
    setStatus("ガイドに2本を合わせて撮影してください","info");
  }catch(e){
    console.error(e);
    setStatus("カメラを起動できません：" + e.message,"warn");
  }
}
function stopCamera(){
...
function drawVisibleVideoToPhoto(){
  const wrap=$("cameraWrap").getBoundingClientRect();
  const targetAspect=wrap.width/wrap.height;
  const vw=video.videoWidth,vh=video.videoHeight;
  if(!vw||!vh)throw new Error("カメラ映像がまだ準備できていません");
...
captureBtn.onclick=async()=>{
  try{
    ...
    $("message").textContent="撮影画像を自動解析します";
    await analyze();
  }catch(e){
...
file.addEventListener("change",e=>{
  ...
  img.onload=async()=>{
    const maxW=1100,s=Math.min(1,maxW/img.naturalWidth);
  ...
  img.onerror=()=>setStatus("画像を読み込めませんでした","ng");
  img.src=url;
});
...
/* 対応ブラウザのみ、黄土色帯の大文字を無料の端末内TextDetectorで読む */
async function nativeBandText(canvas,x0,x1){
  if(!("TextDetector" in window))return null;
  try{
    const W=canvas.width,H=canvas.height;
    const c=document.createElement("canvas");
    c.width=Math.max(1,Math.floor(x1-x0));
    c.height=Math.floor(H*.38);
    c.getContext("2d").drawImage(canvas,x0,H*.16,x1-x0,H*.38,0,0,c.width,c.height);
    const det=new TextDetector();
    const items=await det.detect(c);
    const text=items.map(x=>x.rawValue||"").join(" ").toUpperCase().replace(/\s/g,"");
    if(text.includes("PR-200")||text.includes("PR200"))return"PR";
    if(text.includes("LH-100")||text.includes("LH100"))return"LH";
    return null;
  }catch(e){return null}
}
...
async function analyze(){
  ...
  setProgress(5);setStatus("ラベルを解析しています…","info");await sleep(20);
  ...
  const qL=halfQuality(img,0,mid),qR=halfQuality(img,mid,photo.width);
  const bandL=bandConfidence(img,0,mid),bandR=bandConfidence(img,mid,photo.width);
  const colorL=halfColorEvidence(compsC,compsM,compsY,0,mid),colorR=halfColorEvidence(compsC,compsM,compsY,mid,photo.width);

  const [textL,textR]=await Promise.all([nativeBandText(photo,0,mid),nativeBandText(photo,mid,photo.width)]);
  setProgress(26);
  ...
  async function oneSide(side,t,q,band,colorEvidence,textType,canvas){
    if(textType==="PR"){
      drawMessageCanvas(canvas,"PRIMER","PR-200文字を検出","#eef7ff");
      return{name:"PRIMER",primer:true,reason:"PR-200文字を検出"};
    }
    ...
    if(t){
      ...
      r.shear=k;r.mode=t.mode;r.textType=textType;
      return r;
    }

    if(textType==="LH"){
      drawMessageCanvas(canvas,"要確認","LH-100だが表を検出できない");
      return{name:null,uncertain:true,reason:"LH-100文字あり / 表未検出"};
    }

    /* TextDetectorが使えない端末でも、PRIMERを簡単には断定しない */
    const goodPhoto=q.mean>=145 && q.brightRatio>=.38 && q.darkRatio<=.30 && q.edgeRatio>=.055;
    const goodBand=band>=.045;
    const noInkColors=colorEvidence===0;

    if(goodPhoto && goodBand && noInkColors){
      drawMessageCanvas(canvas,"PRIMER","表なし＋帯あり＋色マスなし","#eef7ff");
      return{name:"PRIMER",primer:true,reason:"表なし / 帯あり / C-M-Y色マスなし"};
    }

    drawMessageCanvas(canvas,"要確認","表を確定できません");
    return{name:null,uncertain:true,reason:`表未検出 band=${band.toFixed(3)} colors=${colorEvidence}`};
  }

  setProgress(45);setStatus("チェック欄を判定中…","info");await sleep(20);

  const rL=await oneSide("L",tL,qL,bandL,colorL,textL,normL);
  const rR=await oneSide("R",tR,qR,bandR,colorR,textR,normR);
  ...
  const dL=rL.primer||rL.uncertain
    ?`左: ${rL.name||"要確認"} / ${rL.reason} / mean=${qL.mean.toFixed(1)} band=${bandL.toFixed(3)} colors=${colorL} text=${textL||"-"}`
    :`左: mode=${rL.mode} 候補=${rL.best.name} score=${rL.best.score.toFixed(3)} / 2位=${rL.second.name} ${rL.second.score.toFixed(3)} / shear=${rL.shear.toFixed(2)} / labels=${gridText(rL)} / text=${textL||"-"}`;

  const dR=rR.primer||rR.uncertain
    ?`右: ${rR.name||"要確認"} / ${rR.reason} / mean=${qR.mean.toFixed(1)} band=${bandR.toFixed(3)} colors=${colorR} text=${textR||"-"}`
    :`右: mode=${rR.mode} 候補=${rR.best.name} score=${rR.best.score.toFixed(3)} / 2位=${rR.second.name} ${rR.second.score.toFixed(3)} / shear=${rR.shear.toFixed(2)} / labels=${gridText(rR)} / text=${textR||"-"}`;

  $("debugText").textContent=dL+"　"+dR;
  setProgress(100);
